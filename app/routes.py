...
def home():
    ...
    posts = Post.query.order_by(Post. likes.desc(), Post.date_posted.desc()).limit(5).all()
    total_posts = Post.query.count()
    total_users = User.query. count()
    return render_template('index.html', posts=posts, total_posts=total_posts, total_users=total_users)

# ============== POSTS - BROWSE WITH SEARCH, FILTER & SORT ==============
@main.route('/posts')
def posts():
    ...
    # Apply sorting
    if sort == 'oldest':
        q = q.order_by(Post.date_posted.asc())
    elif sort == 'popular': 
        q = q.order_by(Post.likes.desc(), Post.date_posted.desc())
    elif sort == 'views': 
        q = q.order_by(Post.views.desc(), Post.date_posted.desc())
    else:  # newest (default)
        q = q.order_by(Post.date_posted.desc())
    
    p = paginate(q, page, per_page=10)
    return render_template('posts.html', posts=p['items'], p=p)
...
def new_post():
    if request.method == 'POST': 
        ...
        post = Post(title=title, content=content, category=category, author=current_user)
        db.session.add(post)
        db.session.commit()
